...
class EsignApiIntegrationTests {
    ...
    @Test void catalogDashboardAndAllModulesAreAvailable() throws Exception {
        ...
        mvc.perform(get("/api/records?module=TEMPLATE").with(httpBasic("operator","operator123"))).andExpect(status().isOk()).andExpect(jsonPath("$.data").isArray());
    }
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void recordCanFollowPrimaryWorkflowAndWritesAudit() throws Exception {
        var result=mvc.perform(post("/api/records").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("""
            {"recordNo":"UT-ESG-001","module":"TEMPLATE","title":"自动化流程验收记录","businessParty":"知华测试组织","owner":"测试专员","amount":12800,"quantity":3,"dueDate":"2026-09-30","riskLevel":"正常","description":"主要流程自动化测试"}
        ...
        mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"ACCEPT\",\"remark\":\"越级操作\"}"))
            .andExpect(status().isConflict());
        mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"SEND\",\"remark\":\"第一步完成\"}"))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.status").value("待签署"));
        mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"ACCEPT\",\"remark\":\"第二步完成\"}"))
            .andExpect(status().isOk()).andExpect(jsonPath("$.data.status").value("签署中"));
    ...
    @Test void draftRecordCanBeUpdatedAndDeleted() throws Exception {
        var result=mvc.perform(post("/api/records").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("""
            {"recordNo":"UT-ESG-002","module":"TEMPLATE","title":"待修改记录","businessParty":"测试部门","owner":"测试员","amount":1000,"quantity":1,"dueDate":"2026-10-10","riskLevel":"正常","description":"用于验证增删改"}
        ...
        mvc.perform(put("/api/records/{id}",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("""
            {"recordNo":"UT-ESG-002","module":"TEMPLATE","title":"已修改记录","businessParty":"测试部门","owner":"测试员","amount":1600,"quantity":2,"dueDate":"2026-10-12","riskLevel":"关注","description":"修改成功"}
